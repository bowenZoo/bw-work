<template>
  <div class="space-y-6">
    ...
    <div>
      ...
      <p class="text-zinc-400 mt-1">系统概览与状态</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div class="bg-zinc-800 rounded-lg p-6">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-3">
            ...
            <div>
              <h3 class="text-white font-medium">LLM 服务</h3>
              <p class="text-zinc-400 text-sm">OpenAI API</p>
            </div>
          </div>
          <span
            :class="[
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-3">
            ...
            <div>
              ...
              <p class="text-zinc-400 text-sm">监控追踪</p>
            </div>
          </div>
          <span
            :class="[
          ...
          >
            {{ status.langfuse_enabled ? '已启用' : status.langfuse_configured ? '已禁用' : '未配置' }}
          </span>
        </div>
      </div>
      ...
      <div class="bg-zinc-800 rounded-lg p-6">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-3">
            <div class="p-2 bg-emerald-900/50 rounded-lg">
              <svg class="w-6 h-6 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
            </div>
            <div>
              <h3 class="text-white font-medium">图像服务</h3>
              <p class="text-zinc-400 text-sm">{{ status.default_image_provider || 'DALL-E' }}</p>
            </div>
          </div>
          <span
            :class="[
          ...
          >
            {{ status.image_configured ? '已配置' : '未配置' }}
          </span>
        </div>
      </div>
    </div>
    ...
    <div class="bg-zinc-800 rounded-lg p-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <router-link
          to="/admin/llm"
          class="p-4 bg-zinc-700/50 rounded-lg hover:bg-zinc-700 transition-colors text-center"
        >
          ...
          <span class="text-zinc-300 text-sm">配置 LLM</span>
        </router-link>
        ...
        >
          ...
          <span class="text-zinc-300 text-sm">Langfuse</span>
        </router-link>
        ...
        >
          ...
          <span class="text-zinc-300 text-sm">图像服务</span>
        </router-link>
        ...
        >
          ...
          <span class="text-zinc-300 text-sm">操作日志</span>
        </router-link>
      </div>
    </div>
    ...
    <div class="bg-zinc-800 rounded-lg p-6">
      ...
      <div v-if="recentLogs.length === 0" class="text-zinc-400 text-center py-8">
        暂无活动记录
      </div>
        ...
        >
          <div class="flex items-center space-x-3">
            ...
            <div>
              ...
              <span class="text-zinc-400">{{ formatAction(log.action) }}</span>
              <span v-if="log.target" class="text-zinc-500 ml-1">({{ log.target }})</span>
            </div>
          </div>
          <span class="text-zinc-500 text-sm">{{ formatTime(log.timestamp) }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="bg-zinc-800 rounded-lg p-6">
      ...
      <div class="grid grid-cols-2 gap-4 text-sm">
        <div class="flex justify-between">
          <span class="text-zinc-400">后端版本</span>
          <span class="text-white">0.1.0</span>
        </div>
        <div class="flex justify-between">
          <span class="text-zinc-400">数据库</span>
          <span class="text-white">SQLite</span>
        </div>
        <div class="flex justify-between">
          <span class="text-zinc-400">API 端口</span>
          <span class="text-white">18000</span>
        </div>
        <div class="flex justify-between">
          <span class="text-zinc-400">管理模块</span>
...
function getActionColor(action: string): string {
  switch (action) {
    ...
    case 'config_update':
    case 'config_delete':
      return 'bg-blue-900/50 text-zinc-400'
    default:
