...
**所有用户可见的 UI 文字必须使用中文**，包括但不限于：

| 元素类型 | 要求 | 示例 |
|----------|------|------|
| 页面标题 | 中文 | ❌ `Dashboard` → ✅ `仪表盘` |
| 按钮文字 | 中文 | ❌ `Submit` → ✅ `提交` |
| 表单标签 | 中文 | ❌ `Username` → ✅ `用户名` |
| 提示信息 | 中文 | ❌ `Loading...` → ✅ `加载中...` |
| 错误消息 | 中文 | ❌ `Invalid input` → ✅ `输入无效` |
| 表格表头 | 中文 | ❌ `Created At` → ✅ `创建时间` |
| 导航菜单 | 中文 | ❌ `Settings` → ✅ `设置` |
| 状态文字 | 中文 | ❌ `Enabled` → ✅ `已启用` |

**例外情况**：
- 技术术语保持原文：`API`、`Token`、`WebSocket`
- 品牌名称保持原文：`OpenAI`、`Langfuse`
- 代码相关内容：变量名、函数名、注释可用英文
